...
function button ($title,$Name,$Owner,$TechContact,$BillingContact,$Password,$RootVolSize,$Phi,$Pii,$InstanceType,$Added) {
...
$DropDownBox.Size = New-Object System.Drawing.Size(130,20) 
...
return $textBox1.Text, $textBox2.Text, $textBox3.Text, $textBox4.Text, $textBox5.Text, $textBox6.Text, $textBox7.Text, $textBox8.Text, $DropDownBox.Text, $textBox10.Text
}
 
$return= button “FHCRC EC2 Server Request Form - Windows” “Host name*” “Owner*(_adm/solarch)” “Tech Contact*(user@fredhutch.org)” “Billing Contact*(user@fredhutch.org)” "Password" "Root Volume Size (GB)" "PHI (Yes,No)" "PII (Yes, No)" "Instance Type" "Custom Flags  (-Region us-west-2 -GrantCritical Yes)"

##################set variables
$Name,$Owner,$TechContact,$BillingContact,$Password,$RootVolSize,$Phi,$Pii,$InstanceType,$Added = $return
[String]$CommPath = "O:\scripts\aws\ec2-provision-instance.ps1"
[String]$CommVar = "-Name $Name -Owner $Owner -TechContact $TechContact -BillingContact $BillingContact -InstanceType $InstanceType -Password $Password -RootVolSize $RootVolSize -Phi $Phi -Pii $Pii $Added"

##################run configure script with parameters
Invoke-Expression "O:\scripts\aws\ec2-provision-instance.ps1 -Name $Name -Owner $Owner -TechContact $TechContact -BillingContact $BillingContact -InstanceType $InstanceType -Password $Password -RootVolSize $RootVolSize -Phi $Phi -Pii $Pii $Added"
##Invoke-Expression "$CommPath $CommVar"

##################variable check (comment out previous command)
##get-variable name,owner,techcontact,billingcontact,password,rootvolsize,phi,pii,instancetype,added,commpath,commvar